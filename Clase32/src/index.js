...
const { infoLog, warnLog, errorLog } = require('./utils/loggers/winston');
// Configuraciones
...
if (config.server_mode === 'CLUSTER') {
	if (cluster.isMaster) {
		infoLog.info(`Master PID -> ${process.pid} is running`);
		//WORKERS
		...
		const server = app.listen(PORT, (err) => {
			// errorLog.error(err);
			infoLog.info(`Server is running on port ${PORT}`);
		});
	}
} else {
	const server = app.listen(PORT, (err) => {
...
app.use('*', (req, res) => {
	warnLog.warn(`${req.method} request for '${req.originalUrl}'`);
...
// artillery quick --count 50 -n 20 http://localhost:8080/api/info > result_artillery.txt
